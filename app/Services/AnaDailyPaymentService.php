...
use App\Repositories\Analyse\AnaDailyPayment\AnaDailyPaymentRepository;

/**
 * 入金集計表（日次） サービス
 */
class AnaDailyPaymentService extends AnalyseServiceBase
{
    private AnaDailyPaymentRepository $repository;
    private $data;

    public function __construct()
    {
        $this->repository = new AnaDailyPaymentRepository();
    }
    ...
    private function appendSubTotal()
    {
        ...
        $groupItems->each(function ($paymentDateItems) {
            $paymentDateItems->each(function ($paymentKbnCdItems) {
    ...
    private function getTotal($subTotals)
    {
        ...
        $total->payment_number = $subTotals->last()->payment_number;
