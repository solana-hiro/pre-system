...
@section('page_title', '入金集計表(月次)')
@section('title', '入金集計表(月次)')
@section('description', '')
@section('keywords', '')
@section('canonical', '')
...
@section('content')
    <form role="search" action="{{ route('analyse.detail.monthly_payment.analyse') }}" method="get">
        @csrf
        <div class="main-area">
            @include('admin.analyse.button_area')
            @include('admin.analyse.def_area', ['code' => '3-007', 'title' => '入金集計表（月次）'])
            <br>
                        ...
                        <thead class="grid_header">
                            <tr>
                                @include('admin.analyse.sortable_th', [
                                    'title' => '入金区分コード',
                                    'column' => 'def_payment_kbns.payment_kbn_cd',
                                ])
                                @include('admin.analyse.sortable_th', [
                                    'title' => '入金区分名',
                                    'column' => 'def_payment_kbns.payment_kbn_name',
                                ])
                                @include('admin.analyse.sortable_th', [
                                    'title' => '銀行名',
                                    'column' => 'bank_name',
                                ...
                                @foreach ($data as $record)
                                    @if (isset($record->total_flag))
                                        <tr class="ana_total_tr">
                                            <td class="grid_wrapper_left" colspan="5">{{ $record->title }}</td>
                                    ...
                                    @else
                                        <tr>
                                            <td class="grid_wrapper_center">{{ $record->payment_kbn_cd }}</td>
                                            ...
                                            <td class="grid_wrapper_left">{{ $record->payment_number }}</td>
                                            <td class="grid_wrapper_left">{{ $record->collect_pay_date }}</td>
                                        </tr>
                                    @endif
                                @endforeach
                            @endisset
