...
@section('page_title', '入金集計表(日次)')
@section('title', '入金集計表(日次)')
@section('description', '')
@section('keywords', '')
@section('canonical', '')
...
@section('content')
    <form role="search" action="{{ route('analyse.detail.daily_payment.analyse') }}" method="get">
        @csrf
        <div class="main-area">
            @include('admin.analyse.button_area')
            @include('admin.analyse.def_area', ['code' => '3-007', 'title' => '入金集計表（日次）'])
            <br>
