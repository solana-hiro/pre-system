...
@section('page_title', '納品先マスタExcel取込')
@section('title', '納品先マスタExcel取込')
@section('description', '')
@section('keywords', '')
@section('canonical', '')
...
@section('content')
	<div class="main-area">
		<form role="search" action="{{ route('master.delivery.mt_delivery_destinations.file.import') }}" method="post" name="mtDeliveryFileImportForm" enctype="multipart/form-data">
	    @csrf
			<div class="button_area">
				<div class="div">
                    <button class="button" type="button" name="delete" data-toggle="modal" data-target="#modal_delete" data-value=""><div class="text_wrapper">仮登録データ削除</div></button>
					@if(Session::has('deliveryDestinationImportError'))
					    <button class="button" type="submit" name="error_output"><div class="text_wrapper">エラー結果出力</div></button>
                    @else
                        <button class="button" type="submit" name="error_output" disabled><div class="text_wrapper">エラー結果出力</div></button>
                    @endif
                    ...
                    <button type="button" data-toggle="modal" data-target="#file_confirm3" data-value="" class="button-2" data-url="" name="update2"><div class="text_wrapper_3">実行する</div></button>
				</div>
	    	</div>
